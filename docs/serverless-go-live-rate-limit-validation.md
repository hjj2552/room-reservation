...
> 이 문서는 Pages Service Binding을 사용하던 시점의 역사적 검증 기록이다. 현재 배포 경계는 Workers Static Assets와 direct `CF-Connecting-IP` 방식으로 대체되었으며 제품 rate-limit 수치와 세션 조회 전 보호 순서는 유지된다.
...
| --- | --- | --- | --- | --- |
| `INGRESS_GUARD_RATE_LIMITER` | 인증 여부와 무관한 모든 `/api/**` | IP별 600/60초 | `<uat-ingress-namespace-id>` | `<production-ingress-namespace-id>` |
...
- Neon branch ID: `<neon-branch-id>`
- 빈 DB: `<uat-database-name>`
- UAT Worker: `<uat-worker-name>`
- 최종 secret 적용 Worker version: `<worker-version-id>`
- Pages branch: `<pages-preview-branch>`
...
- Neon branch ID: `<neon-branch-id>`
- 빈 DB: `<uat-database-name>`
- branch 전용 role: `<uat-database-role>`
- UAT Worker: `<uat-worker-name>`
- 최종 secret 적용 Worker version: `<worker-version-id>`
- Pages branch: `<pages-preview-branch>`
- 성공 Pages deployment: `<pages-deployment-id>`
