...
test('deleted audit rows render two-line snapshots without detail links', async ({ page, request }) => {
  ...
  await expect(rows.nth(0).locator('.audit-snapshot-room')).toHaveText('testing-deleted-room');
  ...
  await expect(rows.nth(1).locator('.audit-snapshot-room')).toHaveText('testing-partial-room');
