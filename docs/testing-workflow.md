...
- 예약 시작·종료 직접 입력은 고정 5분 단위다.
- 시간표 눈금과 상호작용 시작 후보, 운영 시작·종료 시간은 30분 단위다. 최소 예약 시간은 30분 이상이고 최소·최대는 5분의 배수여야 한다.
- 자동 제안과 hover 길이는 `minReservationMinutes`이며 공개 과거 예약은 최종 저장에서 금지하고 관리자 과거 예약은 허용한다.
- 기존 예약에는 새 시간 정책을 소급 적용하지 않는다. 배포 직전 최소·최대 설정의 V3 호환성을 읽기 전용으로 다시 확인한다.
