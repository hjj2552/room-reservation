...
Cloudflare Pages Function proxy를 유지할지 Worker route로 교체할지는 P3에서 결정한다. 어떤 구성을 선택하더라도 프런트 소스의 API 호출 계약은 변경하지 않는다.
...
기존 rate limit 정책을 유지한다.

- 공개 및 비인증 GET 요청: IP별 분당 120회
- 공개 및 비인증 쓰기 요청: IP별 분당 24회
- 인증된 관리자 요청: 기존 정책과 같이 rate limit 우회
- 초과 시 HTTP 429
- 오류 코드 `RATE_LIMIT_EXCEEDED`
- `Retry-After` 응답 header
- 재시도 가능 시간 정보 유지

Worker에서는 임의의 `X-Forwarded-For` 값을 그대로 신뢰하지 않는다. Cloudflare가 제공하는 신뢰 가능한 클라이언트 IP 정보를 HTTP 어댑터에서 판정한다.

Worker isolate 하나의 메모리에만 저장하는 rate limit은 분산된 Worker 실행 환경에서 기존 정책을 보장할 수 없으므로 최종 구현으로 사용하지 않는다.

P3에서는 여러 Worker isolate와 실행 위치 사이에서 필요한 정책을 유지할 수 있는 공유·원자적 rate limit 방식을 검증한다.

구체적인 저장 방식은 P3에서 결정하되 외부 동작과 수치는 변경하지 않는다.
...
다음은 계약서에서 미리 확정하지 않고 P3 최소 기술 검증을 통해 결정한다.
...
P3는 위 항목을 실제 최소 코드로 검증하고 결과에 따라 P4 구현 결정을 내린다.

제품 정책과 외부 API 동작은 P3 기술 선택을 이유로 변경하지 않는다.
...
- rate limit 수치와 429 응답 계약 확인
- `RATE_LIMIT_EXCEEDED`와 `Retry-After` 확인
- 신뢰 가능한 Cloudflare 클라이언트 IP 판정 확인
- isolate-local memory에만 의존하지 않는 rate limit 확인
