...
function expectMigrationFailure(script, databaseUrl) {
...
try {
  ...
  runMigration("scripts/migrate-v6-for-test.ts", v6UpgradeUrl);
  run("docker", [
    "exec", containerName, "psql", "-U", "worker_test", "-d", "worker_v6_upgrade",
    "-v", "ON_ERROR_STOP=1",
    "-c",
    `INSERT INTO rooms(name,capacity,enabled,display_order)
       VALUES ('testing-room-v7-existing',10,true,1);
     INSERT INTO reservation_recurrences(
       room_id,applicant_name,applicant_email,applicant_phone,purpose,start_date,end_date,
       days_of_week,start_time,end_time,conflict_policy,created_by,show_applicant_name
     ) SELECT id,'testing-v7-recurrence',NULL,'010 3333-4444','testing-v7-recurrence',
       DATE '2035-01-08',DATE '2035-01-08','MON','10:00','11:00','FAIL_ALL','admin',false
       FROM rooms WHERE name='testing-room-v7-existing';
     INSERT INTO reservations(
       room_id,recurrence_id,applicant_name,applicant_email,applicant_phone,purpose,
       start_at,end_at,status,source,created_by_actor_type,show_applicant_name
     ) SELECT room_id,id,'testing-v7-reservation',NULL,'010-1111 2222','testing-v7-reservation',
       TIMESTAMPTZ '2035-01-08 01:00:00+00',TIMESTAMPTZ '2035-01-08 02:00:00+00',
       'CONFIRMED','RECURRING_GENERATED','ADMIN',false
       FROM reservation_recurrences WHERE purpose='testing-v7-recurrence';
     INSERT INTO reservations(
     ...
     ) SELECT id,'testing-v7-null',NULL,NULL,'testing-v7-null',
       TIMESTAMPTZ '2035-01-08 03:00:00+00',TIMESTAMPTZ '2035-01-08 04:00:00+00',
       'CONFIRMED','ADMIN_MANUAL','ADMIN',false
       FROM rooms WHERE name='testing-room-v7-existing';
     INSERT INTO reservation_histories(
       reservation_id,action,after_status,actor_type,reservation_room_id,reservation_purpose,
       reservation_applicant_name,reservation_applicant_phone,before_reservation_applicant_phone
     ) SELECT id,'UPDATED',status,'ADMIN',room_id,purpose,applicant_name,
       '010-5555 6666','010 7777-8888'
       FROM reservations WHERE purpose='testing-v7-reservation'`,
  ]);
  runMigration("scripts/migrate.ts", v6UpgradeUrl);
  runMigration("scripts/migrate.ts", v6UpgradeUrl);
  const v6UpgradeState = run("docker", [
    "exec", containerName, "psql", "-U", "worker_test", "-d", "worker_v6_upgrade",
    "--tuples-only", "--no-align",
    "-c",
    `SELECT
       (SELECT count(*) FROM worker_migrations
        WHERE name='007_applicant_phone_normalization_v7') = 1
       AND (SELECT applicant_phone='01011112222'
            FROM reservations WHERE purpose='testing-v7-reservation')
       AND (SELECT applicant_phone IS NULL
            FROM reservations WHERE purpose='testing-v7-null')
       AND (SELECT applicant_phone='01033334444'
            FROM reservation_recurrences WHERE purpose='testing-v7-recurrence')
       AND (SELECT reservation_applicant_phone='01055556666'
              AND before_reservation_applicant_phone='01077778888'
            FROM reservation_histories WHERE reservation_purpose='testing-v7-reservation')
       AND (SELECT count(*) FROM reservations WHERE recurrence_id IS NOT NULL) = 1
       AND (SELECT count(*) FROM pg_constraint
            WHERE conname IN (
  ...
  runMigration("scripts/migrate-v6-for-test.ts", v6InvalidUrl);
  run("docker", [
    "exec", containerName, "psql", "-U", "worker_test", "-d", "worker_v6_invalid",
    "-v", "ON_ERROR_STOP=1",
    "-c",
    `INSERT INTO rooms(name,capacity,enabled,display_order)
       VALUES ('testing-room-v7-invalid',10,true,1);
     INSERT INTO reservations(
       room_id,applicant_name,applicant_email,applicant_phone,purpose,start_at,end_at,
       status,source,created_by_actor_type,show_applicant_name
     ) SELECT id,'testing-v7-invalid',NULL,'010-12ab-5678','testing-v7-invalid',
       TIMESTAMPTZ '2035-01-09 01:00:00+00',TIMESTAMPTZ '2035-01-09 02:00:00+00',
       'CONFIRMED','ADMIN_MANUAL','ADMIN',false
       FROM rooms WHERE name='testing-room-v7-invalid'`,
  ]);
  expectMigrationFailure("scripts/migrate.ts", v6InvalidUrl);
  const invalidState = run("docker", [
    "exec", containerName, "psql", "-U", "worker_test", "-d", "worker_v6_invalid",
    "--tuples-only", "--no-align",
    "-c",
    `SELECT
       (SELECT count(*) FROM worker_migrations
        WHERE name='007_applicant_phone_normalization_v7') = 0
       AND (SELECT applicant_phone='010-12ab-5678'
            FROM reservations WHERE purpose='testing-v7-invalid')
       AND (SELECT count(*) FROM pg_constraint
            WHERE conname IN (
              'chk_reservations_applicant_phone_digits',
              'chk_recurrences_applicant_phone_digits',
  ...
  if (primarySchema !== v4UpgradeSchema) throw new Error("V4 to V7 standalone upgrade schema differs");
