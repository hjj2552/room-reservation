...
- 공개 사용자는 전체 운영 시간·요일 안에서 모든 시간을 같은 방식으로 신청할 수 있습니다.
- 특별 허가 시간·요일은 공개 시간표에 구분해 표시하지 않으며 공개 신청·수정 때 추가 확인 절차를 만들지 않습니다.
- 특별 허가 여부는 예약에 저장하지 않고, 관리자가 승인하는 시점의 최신 운영 설정으로 계산합니다.
- 공개·관리자 자동 시간 제안은 특별 허가 대상이 아닌 첫 미래 후보를 우선하며, 그런 후보가 없으면 첫 특별 허가 후보를 제안합니다.
...
- 관리자가 승인 대기 예약을 승인할 때는 최신 운영 설정을 기준으로 특별 허가 여부를 다시 판정하고 확인합니다.
- 야간 운영, 안전 수칙과 문의 안내는 운영 설정의 `공개 안내`에 작성합니다.

`slotMinutes` API 필드는 프런트엔드 계약 호환을 위해 항상 `5`로 반환되며 운영자가 변경하는 설정은 아닙니다.

신규 설치의 기본 운영시간은 `09:00~21:00`, 운영 요일은 월요일부터 일요일까지입니다. 기존 설치에 V8을 적용할 때는 현재 운영 범위를 유지하되, 특별 허가 기본 범위인 `18:00~21:00`과 토요일·일요일을 포함하는 데 필요한 범위만 확장합니다. 특별 허가 기본 시간은 `18:00~21:00`, 기본 요일은 토요일과 일요일입니다. 운영시간 밖은 공개·관리자 모두 예약할 수 없습니다.

V8은 migration과 새 Worker 배포 사이에도 기존 Worker가 동작하도록 기존 `public_open_time`, `public_close_time`, `public_available_days_of_week` DB 컬럼을 전체 운영 범위 값으로 한 release 동안 유지합니다. 새 API와 애플리케이션은 이 컬럼을 읽거나 수정하지 않으며, 새 Worker 배포가 안정화된 뒤 후속 migration에서 세 legacy 컬럼을 제거해야 합니다.
