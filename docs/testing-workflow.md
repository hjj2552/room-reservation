...
- 운영 설정의 예약 단위는 `5`, `10`, `15`, `30` 중 하나이며 수동 시간 입력 증분으로 사용한다.
- 시간표 눈금과 상호작용 시작 후보, 운영 시작·종료 시간은 30분 단위다. 최소·최대 예약 시간은 현재 예약 단위의 배수여야 한다.
- 자동 제안 길이는 `max(30, minReservationMinutes)`이며 공개 과거 예약은 금지하고 관리자 과거 예약은 허용한다.
- 예약 단위 변경은 기존 예약에 소급 적용하지 않는다. 배포 직전 `slot_minutes=60` 여부를 읽기 전용으로 다시 확인한다.
