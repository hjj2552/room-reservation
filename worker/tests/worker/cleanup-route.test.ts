...
function app(appEnv: "uat" | "prod", enabled: "true" | "false") {
  ...
  return createHttpApp(parseRuntimeConfig({ APP_ENV: appEnv, E2E_CLEANUP_ENABLED: enabled }), {
    products, sessions, adminUsername: "admin", adminPassword: "secret",
  });
}

describe("cleanup route registration", () => {
  it("registers only for non-prod with the explicit flag", async () => {
    expect((await app("uat", "true").request("/api/admin/test-data/e2e/preview")).status).toBe(200);
    expect((await app("uat", "false").request("/api/admin/test-data/e2e/preview")).status).toBe(404);
    expect((await app("prod", "true").request("/api/admin/test-data/e2e/preview")).status).toBe(404);
  });
});
