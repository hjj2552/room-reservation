...
| 변경 영역 | 우선 실행 |
| --- | --- |
| 공개 예약 생성, 충돌, 정책 | `.\gradlew.bat test --tests "*PublicReservationIntegrationTest" --tests "*ReservationConflictServiceTest" --tests "*ReservationPolicyServiceTest"` |
| 관리자 예약 목록/승인/취소 | `.\gradlew.bat test --tests "*AdminReservationIntegrationTest"` |
| 관리자 예약 생성/상세/수정 | `.\gradlew.bat test --tests "*AdminReservationWriteIntegrationTest"` |
| 반복 예약 preview/create/cancel | `.\gradlew.bat test --tests "*RecurrenceIntegrationTest"` |
| 감사 이력 | `.\gradlew.bat test --tests "*ReservationHistoryIntegrationTest"` |
| CSV export | `.\gradlew.bat test --tests "*ReservationCsvExportIntegrationTest"` |
