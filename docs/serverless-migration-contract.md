...
rate limit 구현과 부하 검증은 P4 핵심 재작성의 완료 조건에서 제외하고 실제 Go-Live 전 별도 필수 보안 작업으로 수행한다.

목적은 정확한 전역 과금 제한이 아니라 공개 API의 기본적인 남용 완화다. 모든 Cloudflare 실행 위치에서 완전히 동일한 전역 counter를 제공하는 것은 요구사항이 아니다.

Go-Live 전 후속 작업의 외부 동작 기준은 다음과 같다.
...
Pages Function proxy가 관찰한 원본 IP를 backend Worker가 신뢰할 수 있도록 Service Binding, 서명된 내부 header 또는 그에 준하는 proxy 인증 경계를 먼저 확정한다. isolate-local memory만으로 구현하지 않는다. 구체적인 저장·판정 방식과 허용되는 위치별 근사치는 Go-Live 전 후속 작업에서 결정한다.
...
- Pages Function proxy에서 backend Worker로 전달되는 클라이언트 IP의 인증된 신뢰 경계 확인
- 클라이언트가 주입한 `X-Forwarded-For`를 신뢰하지 않음
...
rate limit 수치와 429, `RATE_LIMIT_EXCEEDED`, `Retry-After`, 신뢰 가능한 Cloudflare 클라이언트 IP 판정, isolate-local memory 비의존 여부는 서버리스 마이그레이션 완료 조건이 아니라 실제 Go-Live 전 필수 완료 조건이다.
