...
- 관리자 절차가 바뀌면 `docs/operator-manual.md`
- 실행 명령, CI, 테스트 방식이 바뀌면 `docs/dev-setup.md`
- 구현 범위나 미지원 범위가 바뀌면 `docs/known-limitations.md`
- E2E 범위가 바뀌면 `docs/admin-e2e.md`
## Local Environment Variables

Local backend runs now require explicit environment values instead of committed
local credentials.

1. Copy `.env.example` to `.env` in the repository root.
2. Copy `backend/src/main/resources/application-local.example.yml` to
   `backend/src/main/resources/application-local.yml` if that local file does
   not exist yet.
3. Fill in `DB_URL`, `DB_USERNAME`, `DB_PASSWORD`, `ADMIN_USERNAME`, and
   `ADMIN_PASSWORD`.
4. Start the backend with the local profile:
...
`application-local.yml` imports the repository-root `.env` file for local
development. Production must not use `.env`; set the same keys in the deployment
platform's environment-variable or secret-management screen.
