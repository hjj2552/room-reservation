...
> **역사적 설계 기준:** 이 계약서는 서버리스 전환을 수행할 때의 요구사항과 완료 gate를 보존합니다. 전환은 완료됐으며 문서 안의 단계·기존 시스템·미완료 표현은 작성 당시 기준입니다. 현재 운영 기준은 [문서 안내](README.md)를 참조하세요.
...
반복 예약은 관리자가 여러 개의 개별 예약을 일괄 생성하는 매크로 성격의 기능이다. 그룹에는 승인·취소 상태가 없으며, 그룹은 존재하거나 하드 삭제된 두 경우만 존재한다.
...
- 반복 예약 그룹 영구 삭제
- 태그와 색상 표시
- 그룹 삭제 시 상태·수정 여부와 관계없이 연결된 모든 개별 예약 영구 삭제
- 그룹과 연결 예약 삭제의 단일 PostgreSQL transaction
- 개별 예약의 `reservation_deleted_id`, 삭제 스냅샷, `DELETED` 감사 이력 보존
- 과거 `RECURRENCE_CANCELLED` 감사 action과 기존 기록의 조회 호환
...
반복 예약 hard-delete Worker가 production에서 안정화된 뒤 적용하는 후속 contract schema 기준은 V5다. V5는 새 Worker가 더 이상 조회하거나 갱신하지 않는 `reservation_recurrences.deleted_at` 열과 `idx_recurrences_deleted_at` 인덱스만 제거한다. 기존 그룹, 연결 예약, 예약 상태와 내용, `recurrence_exception`, 감사 이력은 삭제하거나 변경하지 않는다. V4에서 `deleted_at` 값이 NULL이었던 그룹과 값이 있었던 과거 그룹은 모두 상태 구분 없는 동일한 그룹 row로 보존된다.

V5 down migration은 호환성 롤백을 위한 nullable `deleted_at` 열과 인덱스 구조만 다시 만든다. 열을 제거하면서 소실된 과거 그룹별 `deleted_at` 값은 복원할 수 없으므로, down 이후 모든 기존 row의 해당 값은 NULL이다. 이 구조 롤백은 과거 soft-delete 상태를 복구하는 수단이 아니다.
...
- 관리자 예약 승인과 상태 변경 통과
- 공간·태그 관리 통과
- 반복 예약 미리보기·생성·목록/상세 조회·영구 삭제 통과
- 운영 설정과 감사 이력 기능 통과
- 관리자 예약 CSV 내보내기 통과
